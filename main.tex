...
\begin{enumerate}
    \item Find the logical statement $P(n)$ so that the theorem can be written in the form: ``Show $P(n)$ is true for all $n \in \N_{\geq n_0}$".
    \item Prove the base cases are true: $P(n)$ is true for $n=n_0,\:$.
    \item Write the inductive hypothesis: ``Assume $P(n)$ is true for $n_0\leq n\leq k$".
    \item Write the inductive step: ``$P(n)$ is true for $n=k+1$".
    \item Prove the inductive step is true, usually a direct proof. Remember to utilise the inductive hypothesis.
...
\begin{theorem}[Squeeze Theorem for Sequences]
	Let $\left\{ a_n \right\}_{n=1}^\infty$, $\left\{ b_n \right\}_{n=1}^\infty$, and $\left\{ c_n \right\}_{n=1}^\infty$ be infinite sequences such that $a_n \leq b_n \leq c_n$ for all $n>n_0$. If $\lim_{n\rightarrow\infty}a_n=\lim_{n\rightarrow\infty}c_n=d$, then $\lim_{n\rightarrow\infty}b_n=d$.
\end{theorem}
%
