...
% \begin{figure}[H]
	\begin{mdframed}[style=exampledefaultcols,frametitle={Comparison Test}]
		...
		\end{description}
		If $\displaystyle \sum_{i=i_0}^\infty b_i$ converges
		\begin{equation*}
...
\begin{table}[H]
	...
	\begin{tabular}{>{$\displaystyle}c<{$} | >{$\displaystyle}c<{$}}
		...
		\arccos{\left(\frac{x}{a}\right)}  & -\frac{1}{\sqrt{a^2-x^2}}                         \\
		\arctan{\left(\frac{x}{a}\right)}  & \frac{a}{a^2+x^2}                                 \\
		\arccot{\left(\frac{x}{a}\right)}  & -\frac{a}{a^2+x^2}                                \\
		\arcsec{\left(\frac{x}{a}\right)}  & \frac{a}{\abs*{x} \sqrt{x^2-a^2}}                 \\
		\arccsc{\left(\frac{x}{a}\right)}  & -\frac{a}{\abs*{x} \sqrt{x^2-a^2}}                
	\end{tabular}
\end{table}
\end{document}
