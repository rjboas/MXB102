...
% Execute before loading fancyhdr
\usepackage{geometry}
%% Geometry
\geometry{a4paper}
\geometry{includeheadfoot, margin=15mm}

\input{LaTeX-Submodule/template}
...
% Metadata for README
\newcommand{\unitName}{Abstract Mathematical Reasoning}
\newcommand{\unitTime}{Semester 1, 2021}
\newcommand{\unitCoordinator}{Dr Adrianne Jenner}
...
\begin{multicols}{2}
    ...
    \begin{tcolorboxcols}[title={Integral Test}]
        \textbf{Conditions} Let \(a_i=f(i)\), so that \(f(x)\) is continuous, positive and decreasing on \(\left[a,\:\infty\right)\). % chktex 9
